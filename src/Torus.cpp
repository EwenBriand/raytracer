...
bool Primitive::Torus::hit(const Math::Ray &ray)
{
    ...
    const float E = 4 * A * C + 2 * B * B
        + 4 * R * r
            * (d.getX() * d.getX() + d.getZ() * d.getZ()
                - 2 * d.getY() * d.getY());
    const float F = 4 * A * B + 8 * R * r * o.getY() * d.getY();
    const float G = 4 * A * (R * R - r * r) + B * B
        + 4 * R * r
            * (o.getX() * o.getX() + o.getZ() * o.getZ()
                - 2 * o.getY() * o.getY())
        + 4 * R * r * d.getY() * d.getY()
            * (2 * o.getY() * o.getY() - R * R - r * r);

    // Utiliser la formule de Ferrari pour trouver les racines du polynôme du
    // 4ème degré
    const float a = E / A;
    const float b = D / A;
    const float c = C / A;
    ...
    if (delta > 0) {
        ...
        if (x1 > 0
            && (ray.getOrigin() + ray.getDirection() * x1).getY()
                    * (ray.getOrigin() + ray.getDirection() * x1).getY()
                <= (R * R + r * r - x1 * x1
                    - 2 * r * r
                        * std::sqrt((R * R - x1 * x1) / (R * R + r * r)))) {
            return true;
        }
        if (x2_real > 0
            && (ray.getOrigin() + ray.getDirection() * x2_real).getY()
                    * (ray.getOrigin() + ray.getDirection() * x2_real).getY()
                <= (R * R + r * r - x2_real * x2_real
                    - 2 * r * r
                        * std::sqrt(
                            (R * R - x2_real * x2_real) / (R * R + r * r)))) {
            return true;
        }
    } else if (delta == 0) {
        ...
        if (x2 > 0
            && (ray.getOrigin() + ray.getDirection() * x2).getY()
                    * (ray.getOrigin() + ray.getDirection() * x2).getY()
                <= (R * R + r * r - x2 * x2
                    - 2 * r * r
                        * std::sqrt((R * R - x2 * x2) / (R * R + r * r)))) {
            return true;
        }
    } else {
        ...
        const Math::Vector3D root1 = Math::Vector3D(
            std::pow(alpha * alpha + beta * beta, 1.0f / 6.0f), 0, 0);
        const Math::Vector3D root2 = Math::Vector3D(-alpha / 2, beta / 2, 0);
        const Math::Vector3D root3 = Math::Vector3D(-alpha / 2, -beta / 2, 0);

        // On teste les racines réelles pour savoir si elles sont sur le rayon
        if (root1.getY() == 0 && root1.getX() > 0
            && root1.getX() * root1.getX() + root1.getZ() * root1.getZ()
                <= R * R + r * r) {
            const float y2 = root1.getX() * root1.getX()
                + root1.getZ() * root1.getZ() - R * R - r * r;
            if (std::abs(root1.getY()) <= std::sqrt(4 * r * r * (R * R - y2)
                    / ((R * R + r * r - y2) * (R * R + r * r - y2)
                        - 4 * R * R * r * r))) {
                return true;
            }
        }
        if (root2.getY() == 0 && root2.getX() > 0
            && root2.getX() * root2.getX() + root2.getZ() * root2.getZ()
                <= R * R + r * r) {
            const float y2 = root2.getX() * root2.getX()
                + root2.getZ() * root2.getZ() - R * R - r * r;
            if (std::abs(root2.getY()) <= std::sqrt(4 * r * r * (R * R - y2)
                    / ((R * R + r * r - y2) * (R * R + r * r - y2)
                        - 4 * R * R * r * r))) {
                return true;
            }
        }
        if (root3.getY() == 0 && root3.getX() > 0
            && root3.getX() * root3.getX() + root3.getZ() * root3.getZ()
                <= R * R + r * r) {
            const float y2 = root3.getX() * root3.getX()
                + root3.getZ() * root3.getZ() - R * R - r * r;
            if (std::abs(root3.getY()) <= std::sqrt(4 * r * r * (R * R - y2)
                    / ((R * R + r * r - y2) * (R * R + r * r - y2)
                        - 4 * R * R * r * r))) {
                return true;
            }
...
Math::Point3D Primitive::Torus::getIntersexe() const
{
    return _intersexe;
}

Math::Vector3D Primitive::Torus::getNormal() const
{
